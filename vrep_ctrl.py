...
class sim_run():
    def __init__(self,joint_names,body_names,scene_name,n_time,stepping=True,wall_dims=[3.,9.,0.4]):
    ...
    def set_joint_velos(self,action):
        for i, act in enumerate(action):
            ...
            try:
                ...
                print(f"Error setting velocity for joint ID {self.joint_ids[i]}: {e}")
    ...
    def step_sim(self,action):
        ...
        flag, sim_time, pin, orientation_flag =self.end_sim()
        return flag, state, sim_time, orientation_flag
    ...
    def end_sim(self):
        ...
        if pin[0]>self.final_pos[0]:
            ...
            self.kill_sim()
            return True, sim_time, pin_max, False
        else:
            if pin_max*180/math.pi>90 or self.sim.getSimulationTime()>self.n_time:
                self.kill_sim()
                return True, sim_time, pin_max, True
            else:
                return False, sim_time, pin_max, False
